...
void saveStats (vector<errors> err,string dir, string file_name) {
  ...
  //Show on screen
  std::cout << "Translation error (%): " << t_err/num*100 << std::endl;
  std::cout << "Rotation error (deg/m): " << r_err/num/M_PI*180 << std::endl;
}

bool eval (int seq, string gt_dir, string gt_file_name, string result_dir, string result_file_name, Mail* mail) {
  ...
  for (int32_t i=seq; i<seq+1; i++) {
   
    // file name
    // char file_name[256];
    // sprintf(file_name,"%02d.txt",i);

    string file_name = result_file_name.substr(17,result_file_name.size()-17);
    string seq_str;
    // read ground truth and result poses
    if(i<10)
      seq_str = "0" + std::to_string(seq);
    else
  ...
  if (total_err.size()>0) {
    string avg_file_name = result_file_name.substr(17,result_file_name.size()-21)+"_avg";
    char prefix[16];
    sprintf(prefix,"avg");
...
int32_t main (int32_t argc,char *argv[]) {
  ...
  if (argc!=6) {
    cout << "Usage: ./eval_odometry [seq] [gt_dir] [gt_file_name] [result_dir] [result_file_name]" << endl;
    return 1;
  }
  ...
  string gt_file_name = argv[3];
  string result_dir = argv[4];
  string result_file_name = argv[5];
