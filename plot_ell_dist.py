...
inner_product = np.genfromtxt("inner_product.txt")

rows = inner_product.size

print(inner_product.shape)
ell = np.genfromtxt(sys.argv[1])
print(ell.shape)
rows = min(ell.size, rows)
dist  = np.genfromtxt(sys.argv[2])

rows = min(dist.size, rows)
transform = np.genfromtxt(sys.argv[3])
print(transform.shape)
rows = min(transform.shape[0], rows)
...
iters = np.arange(rows)
ell_line, = plt.plot(iters,ell[:rows], label="ell", linestyle='dashed')
dist_line, = plt.plot(iters,dist[:rows], label="dist", linestyle='solid')
ip_line,= plt.plot(iters, inner_product[:rows], label="inner product")

rerr_line, = plt.plot(iters, r_err[:rows], label="rotation error", linestyle="dotted")
terr_line, = plt.plot(iters, t_err[:rows], label="translation error", linestyle="dotted")
...
plt.legend(handles=[ell_line,  dist_line, ip_line, rerr_line,terr_line ])
plt.show()
